...
class ColumnMetadataComparisonServiceTest {
    ...
    @BeforeEach
    void init() {
        service = new ColumnMetadataComparisonService(sourceRepository, targetRepository, tableProvider);
    }
    ...
    void matchesIdenticalColumns() {
        ...
        when(sourceRepository.getColumnDetails(table)).thenReturn(List.of(c));
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(c));
    ...
    void matchesEquivalentIntTypes() {
        when(sourceRepository.getColumnDetails(table))
                .thenReturn(List.of(col("ID", "INTEGER", null, false, false)));
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("ID", "BIGINT", null, false, false)));
    ...
    void matchesCharacterAgainstBinaryType() {
        ...
        when(targetRepository.getColumnDetails(table))
    ...
    void matchesCharacterAgainstChar() {
        ...
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("C", "CHAR", null, true, false)));
    ...
    void returnsFalseWhenTypeMismatch() throws IOException {
        ...
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("C", "DECIMAL", null, true, false)));

        assertThat(service.shouldDDLMatch(table)).isFalse();
        assertThat(ReportTestSupport.readReport(ReportType.COLUMN_METADATA_COMPARISON)).contains("Column Type Mismatch");
    }

    @Test
    void returnsFalseWhenNullabilityDiffers() throws IOException {
        when(sourceRepository.getColumnDetails(table)).thenReturn(List.of(col("C", "INT", null, true, false)));
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("C", "INT", null, false, false)));

        assertThat(service.shouldDDLMatch(table)).isFalse();
        assertThat(ReportTestSupport.readReport(ReportType.COLUMN_METADATA_COMPARISON)).contains("Nullability Mismatch");
    }

    @Test
    void returnsFalseWhenAutoIncrementDiffers() throws IOException {
        when(sourceRepository.getColumnDetails(table)).thenReturn(List.of(col("ID", "INT", null, false, true)));
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("ID", "INT", null, false, false)));
    ...
    void returnsFalseWhenDefaultValueDiffers() throws IOException {
        when(sourceRepository.getColumnDetails(table)).thenReturn(List.of(col("ST", "VARCHAR", "X", false, false)));
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("ST", "VARCHAR", "Y", false, false)));
    ...
    @Test
    void writesSuccessAndReturnsFalseWhenSourceHasNoColumns() throws IOException {
        when(sourceRepository.getColumnDetails(table)).thenReturn(List.of());
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of());

        assertThat(service.shouldDDLMatch(table)).isFalse();
        assertThat(ReportTestSupport.readReport(ReportType.COLUMN_METADATA_COMPARISON)).contains("There is no column");
    }

    @Test
    void findDiffAggregatesCounts() throws IOException {
        when(tableProvider.getTables()).thenReturn(List.of(table));
        when(sourceRepository.getColumnDetails(table)).thenReturn(List.of(col("X", "INT", null, false, false)));
        when(targetRepository.getColumnDetails(table)).thenReturn(List.of(col("X", "INT", null, false, false)));
